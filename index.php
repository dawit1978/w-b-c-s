...
class Index extends Controller {
    ...
    public function __construct() {
        ...
        // var_dump($this->department_model->getDept_Name(1));
    ...
    public function login() {
        ...
        if ($this->administrator_model->isExist(trim($_POST['username']), /*md5(*/trim($_POST['password'])))/*)*/ {
            ...
            if ($this->administrator_model->Account_Type == "Admin") {
                ...
                // Session::set_surname($this->administrator_model->Surname);
                // Session::set_firstname($this->administrator_model->First_Name);
                // Session::set_middlename($this->administrator_model->Middle_Name);
                // Session::set_emailAdd($this->administrator_model->email_add);
                // Session::set_photo($this->administrator_model->Picture);
                // echo ("var dump");
                // echo ($this->administrator_model->Surname);
                // echo ($this->administrator_model->First_Name);
                // echo ($this->administrator_model->Middle_Name);
                // echo ($this->administrator_model->email_add);
                // echo ($this->administrator_model->Picture);
                Session::set_surname("Tester surname");
                Session::set_firstname("First Name");
                Session::set_middlename("Middle Name");
                Session::set_emailAdd("Email Address");
                Session::set_photo("");
    ...
    public function changePassword($username, $hash) {
        ...
        if (isset($_POST['GO'])) {
            ...
            if (!$query) {
                $this->template->setAlert('Invalid URL!!....!' . mysqli_error(), Template::ALERT_ERROR);
            }
    ...
    // this is the disabled ForgotPass function 
    public function ForgotPass(){}

    // // enable this function if you have the mailer configured 
    // public function ForgotPass() {
    ...
    public function student_register() {
        if (isset($_POST['Save'])) {
            ...
            $stud_id = trim( $_POST['number']. "-" .$_POST['stud_id'] );
            ...
            if (!$this->registered_student_model->isValid($stud_id, trim($_POST['surname']), trim($_POST['firstname']))) {
                $hash = crypt(($_POST['number']. "-" . $_POST['stud_id'] . $_POST['emailAdd']), 'wrawehydrufmjhyaswtgf');
                $course_id = $this->courses_model->getCourseID(trim($_POST['course']));
                
                $this->administrator_model->insertStudent(( $_POST['number']. "-" . $_POST['stud_id']), /*md5(*/($_POST['password'])/* )*/, trim($_POST['surname']), trim($_POST['firstname']), trim($_POST['middleName']), $_POST['emailAdd'], $Picture, $hash);
                
                // var_dump($_POST, $Picture, $hash,( $_POST['number']. "-" . $_POST['stud_id']));
                ...
                $this->stud_model->insert(($_POST['number']. "-" . $_POST['stud_id']), ($_POST['gender']), ($_POST['year_level']), ($_POST['program']), ($_POST['section']), $course_id, ($_POST['Status']));


                $this->administrator_model->verifyStudent(($_POST['number']. "-" . $_POST['stud_id']),$hash);


                // $verificationLink = HOST . "/index.php?action=verify&username=" . ($_POST['number']. "-" . $_POST['stud_id']) . "&hash=" . $hash;
...
//          echo "Section: " .$_POST['sign_name'] ."<br/>";
    ...
    public function student_registrationForm($is_register = false, $isExist = false) {
        ...
        $listOfYear = $this->school_year_model->getSchool_Year();
    ...
    private function setSession($account_type) {
        $result = mysqli_fetch_array($this->administrator_model->getUser(trim($_POST['username']), /*md5(*/trim($_POST['password'])))/*)*/;
