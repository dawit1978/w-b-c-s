...
class Requirements_Model extends Model{
    ...
    public function addRequirement($title, $description, $signatory, $sysemID, $visibility, 
        ...
        $this->query = mysqli_query($this->connectdb,"INSERT INTO `socs`.`requirements` (`Requirement_ID`, `Title`, `Description`, 
    ...
    public function editRequirement($requirementID, $title, $description, $signatory, $sysemID, $visibility, 
        ...
        $this->query = mysqli_query($this->connectdb,"UPDATE `socs`.`requirements` SET 
                                    `Title` = '$title', 
    ...
    public function getRequirement($requirementID){
        $this->query = mysqli_query($this->connectdb,"SELECT `Title`,`Description`,`Signatory_ID`,`schoolyearsem`.`School_Year`, 
    ...
    public function filterRequirements($sign_id, $sysem_id, $page, $search){
        $this->query = mysqli_query($this->connectdb,"select * from requirements where Signatory_ID = '$sign_id' AND sy_sem_id = '$sysem_id' AND
                                    (Title like '%$search%')
                                    order by Title
    ...
    public function deleteRequirements($key){
        mysqli_query($this->connectdb,"delete from requirements where Requirement_ID = '$key'");
