...
class SignatorialList_Model extends Model {
    ...
    public function filter($Tdept_name, $Tsign_name, $Tpage, $used_for) {
        $this->query = mysqli_query($this->connectdb,"select signatories.signatory_id, signatories.signatory_name  from signatorialList
...
//        $filter = array();
//        $this->query = mysqli_query($this->connectdb,"select signatories.signatory_id from signatorialList
...
//        $filter = array();
//        $this->query = mysqli_query($this->connectdb,"select signatories.signatory_name from signatorialList
//                                    inner join signatories 
//                                    on signatorialList.signatory_id = signatories.signatory_id
//                                    inner join departments
//                                    on signatorialList.department_id = departments.department_id
...
//    }
    ...
    public function update($dept_ID, $sign_ID, $newSign_ID) {
        mysqli_query($this->connectdb,"UPDATE signatoriallist SET Signatory_ID=$newSign_ID
                    WHERE Department_ID=$dept_ID and Signatory_ID=$sign_ID");
    }

    public function deleteSignatorial($dept_ID, $sign_ID) {
        mysqli_query($this->connectdb,"delete from signatoriallist where Department_ID = '$dept_ID' and Signatory_ID = '$sign_ID'");
    }

    public function insert($dept_ID, $sign_ID, $usable) {
        mysqli_query($this->connectdb,"INSERT INTO `socs`.`signatoriallist` (`Department_ID`, `Signatory_ID`, `Usability`) 
    ...
    public function getListofSignatoryID() {
        $rowInfo = array();
        $this->query = mysqli_query($this->connectdb,"select signatory_ID from signatories order by signatory_name");
    ...
    public function getListOfDept_underSignName($signID) {
        $filter = array();
        $this->query = mysqli_query($this->connectdb,"select departments.department_name from signatorialList
                                    inner join departments on signatorialList.department_id = departments.department_id
    ...
    public function getListOfDept_underSignNameID($signID) {
        $filter = array();
        $this->query = mysqli_query($this->connectdb,"select departments.department_id from signatorialList
                                    inner join signatories on signatorialList.signatory_id = signatories.signatory_id
                                    inner join departments on signatorialList.department_id = departments.department_id
                                    where signatorialList.signatory_id = '$signID'");
    ...
    public function getSignatory_ID($sign_name){
        $this->query = mysqli_query($this->connectdb,"select Signatory_ID from signatories where Signatory_Name like '%$sign_name%'");
        $row = mysqli_fetch_array($this->query);
    ...
    public function getUsed_For($sign_name){
        $this->query = mysqli_query($this->connectdb,"select Used_For from signatories where Signatory_Name like '%$sign_name%'");
        $row = mysqli_fetch_array($this->query);
    ...
    */
    ...
    public function getListofSignatoryByDept($dept_id, $used_for) {
        $this->query = mysqli_query($this->connectdb,"select signatoriallist.Signatory_ID, signatory_name from signatoriallist
    ...
    public function getListOfCourse_Sign($signID){
        $this->query = mysqli_query($this->connectdb,"select courses.course_name from signatoriallist
                                    inner join departments on (signatoriallist.Department_ID = departments.Department_ID)  
    ...
    public function getListOfCourse_SignID($signID){
        $this->query = mysqli_query($this->connectdb,"select courses.course_id from signatoriallist
                                    inner join departments on (signatoriallist.Department_ID = departments.Department_ID)  
                                    inner join courses on (departments.department_id = courses.department_id)
                                    where signatoriallist.signatory_id = '$signID'");
    ...
    public function isExist($dept_id, $sign_id, $used_for){
        $this->query = mysqli_query($this->connectdb,"SELECT * from signatoriallist
                                inner join Signatories on Signatories.`Signatory_ID` = signatoriallist.`Signatory_ID`
...
//        $rowInfo = array();
//        $this->query = mysqli_query($this->connectdb,"select signatory_name from signatoriallist
...
//        $rowInfo = array();
//        $this->query = mysqli_query($this->connectdb,"select signatoriallist.Signatory_ID from signatoriallist
//                                    inner join departments on departments.Department_ID = signatoriallist.Department_ID
//                                    inner join signatories on signatories.Signatory_ID = signatoriallist.Signatory_ID
//                                    where departments.Department_ID = '$dept_id'");
//        
