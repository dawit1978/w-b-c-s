...
class ClearanceStatus extends Model{
    ...
    public function getClearanceStatus($reqNum, $clearedNum){
        $this->query = mysqli_query($this->connectdb,"select if('$reqNum' = '$clearedNum', 'Cleared', 'Not Cleared') as status");
    ...
    public function getStudent_NumberOfRequirements($student_username, $sign_id, $sy_id){
        $this->query = mysqli_query($this->connectdb,"select count(student) as Count from clearancestatus
    ...
    public function getRequirementList($studentID, $signatoryID, $sysemID){
        ...
        $this->query = mysqli_query($this->connectdb,"select Requirement_ID, Title, Description, Requirement_Type, Prerequisite_Signatory from requirements 
    ...
    private function getRequirementClearanceStatus($studentID, $requirementID){
        $subquery = mysqli_query($this->connectdb,"select Cleared from clearancestatus where student='$studentID' and requirement_id='$requirementID' limit 0,1");
        $count = mysqli_num_rows($subquery);
        if ($count==0){
            return 'Not Cleared';
        }else{
            $row = mysqli_fetch_array($subquery);
            return $row[0];
        }
    }
    ...
    public function setRequirementClearanceStatus($studentID, $requirementID, $status){
        if($this->requirementClearanceStatusExists($studentID, $requirementID)){
            ...
            mysqli_query($this->connectdb,"UPDATE `socs`.`clearancestatus` SET `Cleared` = '$status' WHERE `clearancestatus`.`Requirement_ID` = '$requirementID' AND `clearancestatus`.`Student` = '$studentID'");
        }else{
            //var_dump("INSERT INTO `socs`.`clearancestatus` (`Requirement_ID`, `Student`, `Cleared`) VALUES ('$requirementID', '$studentID', '$status')");
            mysqli_query($this->connectdb,"INSERT INTO `socs`.`clearancestatus` (`Requirement_ID`, `Student`, `Cleared`) VALUES ('$requirementID', '$studentID', '$status')");
        }
    ...
    private function requirementClearanceStatusExists($studentID, $requirementID){
        $subquery = mysqli_query($this->connectdb,"select * from clearancestatus where requirement_id='$requirementID' and  student='$studentID'");
        $count = mysqli_num_rows($subquery);
        return $count > 0 ? TRUE:FALSE;
    }
    
    public function isStudentUnderSpecifiedSignatory($username, $sigID){
        $this->query = mysqli_query($this->connectdb,"select * from students
